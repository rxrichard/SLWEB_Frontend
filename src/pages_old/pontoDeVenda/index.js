...
export default class PDV extends Component {
  ...
  switchPDV(num) {
    ...
    this.setState({ update: this.state.PDV[num] })
    this.setState({
      update: {
        ...this.state.update,
  ...
  render() {
    return (
      ...
      <Panel>
            ...
            >
              {this.state.PDV.map((pdv, i) => (
                <option value={i} key={pdv.AnxId}>
                  {pdv.AnxDesc + ', CNPJ: ' + pdv.CNPJ + ' - Ponto de Venda número: ' + pdv.PdvId}
                </option>
              ))}
            </Select>
          </Campo>
        ...
        <Button
          onClick={() => {
            window.location.assign('/pontosdevenda/novo')
          }}
        >
